...
export default function OutcomeSheet({ item, onClose, onLogged, submitLabel = "Save outcome" }: Props) {
  ...
  return (
    <div className="sheet-backdrop" onClick={() => !busy && onClose()}>
      <div className="sheet" role="dialog" aria-modal="true" aria-label={`Log outcome for ${item.company}`} onClick={(e) => e.stopPropagation()}>
        <div className="sheet-head">
          <strong>Log outcome · {item.company}</strong>
          <button className="link-button" onClick={onClose} disabled={busy}>Cancel</button>
        </div>
